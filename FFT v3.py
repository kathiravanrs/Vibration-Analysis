...
abs_fourier_x = [(abs(x) / len(fourier_x_list))**2 for x in fourier_x_list]


# FFT is applied on the Y-Axis data, and then normalised
fourier_y_list = fft(y_list)
abs_fourier_y = [(abs(y) / len(fourier_y_list))**2 for y in fourier_y_list]
...
print(final_fourier_y)
print(sorted(final_fourier_y, reverse=True))
